...
RUN apt-get update && \
    apt-get install build-essential ninja-build curl cmake git python3 python3-dev python3-pip -y --no-install-recommends && \
    apt-get clean autoclean -y

# install cuda build tools
RUN apt-get install nvidia-cuda cuda-cudart-10-2 cuda-cufft-10-2 cuda-curand-10-2 cuda-cusolver-10-2 cuda-cusparse-10-2 cuda-npp-10-2 cuda-nvgraph-10-2 \ 
